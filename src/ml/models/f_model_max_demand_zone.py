...
Ejemplo:
    uv run -m src.ml.models.f_model_max_demand_zone
"""
...
def split_timewise_samples(
    ...
    if train.empty or val.empty or test.empty:
        raise ValueError(
            "El split temporal ha dejado un bloque vacio. Ajusta train_frac/val_frac."
        )
...
def encode_with_global_classes(
    y: pd.Series,
...
def evaluate_and_persist_model(
    ...
    y_train = encode_with_global_classes(train_df["target_zone_id"], class_labels)
    y_val = encode_with_global_classes(val_df["target_zone_id"], class_labels)
    y_test = encode_with_global_classes(test_df["target_zone_id"], class_labels)
