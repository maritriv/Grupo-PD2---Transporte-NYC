...
def _build_pipeline_with_config(
    ...
    *,
    ...
    gbt_max_depth: int,
    gbt_step_size: float,
    gbt_subsampling_rate: float,
    gbt_feature_subset_strategy: str,
    gbt_max_bins: int,
    gbt_min_instances_per_node: int,
    gbt_seed: int,
    ...
    gbt = GBTRegressor(
        ...
        featureSubsetStrategy=gbt_feature_subset_strategy,
        maxBins=gbt_max_bins,
        minInstancesPerNode=gbt_min_instances_per_node,
        seed=gbt_seed,
    )
    stages.append(gbt)
...
def _resolve_gbt_training_params(
    *,
    gbt_max_iter: int | None,
    gbt_max_depth: int | None,
    gbt_step_size: float | None,
    gbt_subsampling_rate: float | None,
    gbt_max_bins: int | None,
    gbt_min_instances_per_node: int | None,
    gbt_feature_subset_strategy: str | None,
) -> dict[str, Any]:
    # Defaults conservadores orientados a WSL: GBT es más lento que RF, así que
    # maxIter moderado, maxDepth bajo y stepSize pequeño para estabilidad.
    defaults = {
        "gbt_max_iter": 50,
        "gbt_max_depth": 4,
        "gbt_step_size": 0.1,
        ...
        "gbt_max_bins": 16,
        "gbt_min_instances_per_node": 2,
        "gbt_feature_subset_strategy": "sqrt",
    }
    return {
        "gbt_max_iter": defaults["gbt_max_iter"] if gbt_max_iter is None else gbt_max_iter,
        "gbt_max_depth": defaults["gbt_max_depth"] if gbt_max_depth is None else gbt_max_depth,
        "gbt_step_size": defaults["gbt_step_size"] if gbt_step_size is None else gbt_step_size,
        "gbt_subsampling_rate": (
            defaults["gbt_subsampling_rate"] if gbt_subsampling_rate is None else gbt_subsampling_rate
        ),
        "gbt_max_bins": defaults["gbt_max_bins"] if gbt_max_bins is None else gbt_max_bins,
        "gbt_min_instances_per_node": (
            defaults["gbt_min_instances_per_node"] if gbt_min_instances_per_node is None else gbt_min_instances_per_node
        ),
        "gbt_feature_subset_strategy": (
            defaults["gbt_feature_subset_strategy"] if gbt_feature_subset_strategy is None else gbt_feature_subset_strategy
        ),
    }
...
def _build_tuning_candidates(
    *,
    max_trials: int,
    max_iter_values: list[int] | None,
    max_depth_values: list[int] | None,
    step_size_values: list[float] | None,
    subsampling_values: list[float] | None,
    max_bins_values: list[int] | None,
    base_params: dict[str, Any],
) -> list[dict[str, Any]]:
    # Grid por defecto optimizado para GBT (maxDepth bajo, maxIter amplio pero conservador).
    defaults = {
        "gbt_max_iter": [30, 50, 70],
        "gbt_max_depth": [3, 4, 5],
        "gbt_step_size": [0.05, 0.1],
        "gbt_subsampling_rate": [0.7, 0.8],
        "gbt_max_bins": [16],
    }
    grid = {
        "gbt_max_iter": max_iter_values or defaults["gbt_max_iter"],
        "gbt_max_depth": max_depth_values or defaults["gbt_max_depth"],
        "gbt_step_size": step_size_values or defaults["gbt_step_size"],
        "gbt_subsampling_rate": subsampling_values or defaults["gbt_subsampling_rate"],
        "gbt_max_bins": max_bins_values or defaults["gbt_max_bins"],
    }
...
def run_gbt_stress(
    ...
    gbt_max_iter: int | None = None,
    gbt_max_depth: int | None = None,
    gbt_step_size: float | None = None,
    gbt_subsampling_rate: float | None = None,
    gbt_max_bins: int | None = None,
    gbt_min_instances_per_node: int | None = None,
    gbt_feature_subset_strategy: str | None = None,
    tune_hyperparams: bool = False,
    tune_metric: str = "rmse",
    tune_max_trials: int = 8,
    tune_train_sample_frac: float = 0.25,
    tune_val_sample_frac: float = 0.50,
    tune_max_iter_values: list[int] | None = None,
    tune_max_depth_values: list[int] | None = None,
    tune_step_size_values: list[float] | None = None,
    ...
    print_stage("ML GBT (SPARK)", "Regresion de stress con GBTRegressor en Spark", color="green")
    ...
    if gbt_step_size is not None and not (0 < gbt_step_size <= 1):
        raise ValueError("gbt_step_size debe estar en (0, 1].")

    spark = get_spark_session(shuffle_partitions=shuffle_partitions)
    gbt_params = _resolve_gbt_training_params(
        ...
        gbt_min_instances_per_node=gbt_min_instances_per_node,
        gbt_feature_subset_strategy=gbt_feature_subset_strategy,
    )
    ...
    try:
        ...
        if tune_hyperparams:
            ...
            for i, params in enumerate(candidates, start=1):
                console.print(
                    ...
                    f"| maxIter={params['gbt_max_iter']} depth={params['gbt_max_depth']} "
                    f"stepSize={params['gbt_step_size']} subsample={params['gbt_subsampling_rate']} "
                    f"bins={params['gbt_max_bins']}"
                )
                trial_pipeline, _ = _build_pipeline_with_config(
                    ...
                    gbt_feature_subset_strategy=gbt_params["gbt_feature_subset_strategy"],
                    gbt_max_bins=int(params["gbt_max_bins"]),
                    gbt_min_instances_per_node=gbt_params["gbt_min_instances_per_node"],
                    gbt_seed=42,
                )
            ...
            tuning_trials_fp = outputs_path / "gbt_stress_spark_tuning_trials.csv"
            ...
            tuning_summary = {
                ...
                "best_params": selected_gbt_params,
                "trials_csv": str(tuning_trials_fp),
            }
            tuning_report_fp = outputs_path / "gbt_stress_spark_tuning_report.json"
            save_json(tuning_summary, tuning_report_fp)

            console.print(
                "[green]Mejor trial[/green] "
                f"| {tune_metric}={best_trial['score_value']:.6f} "
                f"| params={selected_gbt_params}"
            )
        else:
            tuning_summary = {
                "enabled": False,
                "selected_params": selected_gbt_params,
            }
        ...
        def _build_pipeline_selected() -> tuple[Pipeline, list[str]]:
            return _build_pipeline_with_config(
                ...
                cat_cols=cat_cols,
                one_hot_encode_cats=one_hot_encode_cats,
                gbt_max_iter=selected_gbt_params["gbt_max_iter"],
                gbt_max_depth=selected_gbt_params["gbt_max_depth"],
                gbt_step_size=selected_gbt_params["gbt_step_size"],
                gbt_subsampling_rate=selected_gbt_params["gbt_subsampling_rate"],
                gbt_feature_subset_strategy=selected_gbt_params["gbt_feature_subset_strategy"],
                gbt_max_bins=selected_gbt_params["gbt_max_bins"],
                gbt_min_instances_per_node=selected_gbt_params["gbt_min_instances_per_node"],
                gbt_seed=42,
            )

        console.print(
            "[cyan]Entrenando modelo base (fit en train)[/cyan] "
            f"| maxIter={selected_gbt_params['gbt_max_iter']} "
            f"| maxDepth={selected_gbt_params['gbt_max_depth']} "
            f"| stepSize={selected_gbt_params['gbt_step_size']} "
            f"| subsamplingRate={selected_gbt_params['gbt_subsampling_rate']} "
            f"| maxBins={selected_gbt_params['gbt_max_bins']} "
        ...
        if fit_all_data:
            ...
            model_all_fp = outputs_path / "gbt_stress_spark_model_all_data"
        ...
        model_fp = outputs_path / "gbt_stress_spark_model"
        ...
        importance_fp = outputs_path / "gbt_stress_spark_feature_importance.csv"
        importance_df.to_csv(importance_fp, index=False)

        report = {
            "model": "GBTRegressor_spark",
            ...
            "training_params": {
                "gbt_max_iter": int(selected_gbt_params["gbt_max_iter"]),
                "gbt_max_depth": int(selected_gbt_params["gbt_max_depth"]),
                "gbt_step_size": float(selected_gbt_params["gbt_step_size"]),
                "gbt_subsampling_rate": float(selected_gbt_params["gbt_subsampling_rate"]),
                "gbt_max_bins": int(selected_gbt_params["gbt_max_bins"]),
            },
            "training_protocol": {
                "tuning": "train -> val",
        ...
        report_fp = outputs_path / "gbt_stress_spark_report.json"
        ...
        console.print("\n[bold cyan]=== RESULTADO FINAL (TEST) ===[/bold cyan]")
        console.print(f"[yellow]Target:[/yellow] {target_col}")
        console.print(f"[yellow]MAE:[/yellow] {metrics_test.get('mae', 'N/A'):.6f}")
        console.print(f"[yellow]RMSE:[/yellow] {metrics_test.get('rmse', 'N/A'):.6f}")
        console.print(f"[yellow]R²:[/yellow] {metrics_test.get('r2', 'N/A'):.6f}")
        console.print(f"[yellow]Modelo base entrenado en:[/yellow] {final_model_training_data}")

        print_done("GBT SPARK COMPLETADO")
        return report
    finally:
...
def main() -> None:
    p = argparse.ArgumentParser(description="Entrena GBTRegressor con Spark ML.")
    p.add_argument(
        "--input-dir",
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Numero de iteraciones (arboles) del GBT. Si no se pasa, usa el valor por defecto del script.",
    )
    p.add_argument(
        "--gbt-max-depth",
        type=int,
    ...
    p.add_argument(
        ...
        type=float,
        default=None,
        help="Learning rate (stepSize). Si no se pasa, usa el valor por defecto del script.",
    )
    p.add_argument(
        "--gbt-subsampling-rate",
        type=float,
        default=None,
        help="Subsampling por iteracion. Si no se pasa, usa el valor por defecto del script.",
    )
    p.add_argument(
        "--gbt-max-bins",
        type=int,
    ...
    p.add_argument(
        "--gbt-min-instances-per-node",
        type=int,
        default=None,
        help="Minimo de instancias por nodo. Si no se pasa, usa el valor por defecto del script.",
    )
    p.add_argument(
        "--gbt-feature-subset-strategy",
        default=None,
        help="Estrategia de subset de features (auto, sqrt, onethird). Si no se pasa, usa el valor por defecto del script.",
    )
    p.add_argument(
        "--tune",
        action="store_true",
    ...
    p.add_argument(
        "--tune-max-iter-values",
        default=None,
        help="Lista CSV para tuning de maxIter (ej: 80,100,120).",
    )
    p.add_argument(
        "--tune-max-depth-values",
        default=None,
        help="Lista CSV para tuning de maxDepth (ej: 4,5,6).",
    )
    p.add_argument(
        "--tune-step-size-values",
        default=None,
        help="Lista CSV para tuning de stepSize (ej: 0.05,0.1,0.2).",
    )
    p.add_argument(
        "--tune-subsampling-values",
        default=None,
        help="Lista CSV para tuning de subsamplingRate (ej: 0.7,0.8).",
    )
    p.add_argument(
        "--tune-max-bins-values",
        default=None,
        help="Lista CSV para tuning de maxBins (ej: 16,32).",
    )
    p.add_argument(
        "--skip-train-metrics",
        action="store_true",
    ...
    args = p.parse_args()
    tune_max_iter_values = parse_csv_values(args.tune_max_iter_values, int)
    tune_max_depth_values = parse_csv_values(args.tune_max_depth_values, int)
    tune_step_size_values = parse_csv_values(args.tune_step_size_values, float)
    ...
    run_gbt_stress(
        ...
        gbt_step_size=args.gbt_step_size,
        gbt_subsampling_rate=args.gbt_subsampling_rate,
        gbt_max_bins=args.gbt_max_bins,
        gbt_min_instances_per_node=args.gbt_min_instances_per_node,
        gbt_feature_subset_strategy=args.gbt_feature_subset_strategy,
        ...
        tune_max_iter_values=tune_max_iter_values,
        tune_max_depth_values=tune_max_depth_values,
        tune_step_size_values=tune_step_size_values,
