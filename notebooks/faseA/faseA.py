...
def explorar_archivo_fase_a(filepath, servicio):
    try:
        tmp_df = pd.read_parquet(filepath, engine='pyarrow').head(1)
        ...
        df = pd.read_parquet(filepath, columns=columnas_necesarias, engine='pyarrow')
...
def main():
    ...
    if resumen_total:
        ...
        console.print(f"  - {csv_path}.csv")
        console.print(f"  - {md_path}.md")
